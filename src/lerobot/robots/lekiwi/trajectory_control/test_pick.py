...
"""
Pick action standalone test script.

Detects pink object and executes pick operation:
1. Detect pink object -> calculate distance
2. Play approach trajectory proportionally
3. Close gripper (target = 0)
4. Return to initial state

Usage:
    python test_pick.py
    python test_pick.py --ip 192.168.20.199
"""
...
from .color_detection import convert_to_bgr, detect_pink
...
def test_pick(remote_ip: str | None = None, dry_run: bool = False):
    """
    Execute pick action test.
    ...
    print("=" * 50)
    print("Pick Action Test")
    print("=" * 50)

    # Load approach trajectory
    print(f"\nLoading approach_pick trajectory: {config.datasets.approach_pick}")
    approach_trajectory = load_trajectory_from_dataset(config.datasets.approach_pick)
    ...
    try:
        ...
        if result is None:
            print("Pink object not detected!")
            return
        ...
        if pixel_distance < min_dist or pixel_distance > max_dist:
            ...
            if pixel_distance < min_dist:
                print(f"  Object is too CLOSE (min: {min_dist:.1f}px)")
            else:
                print(f"  Object is too FAR (max: {max_dist:.1f}px)")
        ...
        if dry_run:
            print("\n[DRY RUN] Would execute pick action with the above parameters.")
            return

        # Confirm execution
        response = input("\nExecute pick action? (y/n): ")
        if response.lower() != "y":
            print("Aborted.")
            return
        ...
        # 1. Approach (with shoulder pan angle set to object direction)
...
def main():
    parser = argparse.ArgumentParser(description="Pick action standalone test")
    ...
    test_pick(args.ip, args.dry_run)
