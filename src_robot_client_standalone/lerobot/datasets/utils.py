...
def hf_transform_to_torch(items_dict: dict[str, list[Any]]) -> dict[str, list[torch.Tensor | str]]:
    ...
    for key in items_dict:
        ...
        if isinstance(first_item, PILImage.Image):
            items_dict[key] = [_pil_image_to_tensor(img) for img in items_dict[key]]
...
def _pil_image_to_tensor(image: PILImage.Image) -> torch.Tensor:
    """Convert a PIL image to a channel-first float tensor in [0, 1] without torchvision."""
    array = np.asarray(image, dtype=np.float32)
    if array.ndim != 3 or array.shape[2] not in (1, 3, 4):
        raise ValueError(f"Expected a 3D array with channels last, got shape {array.shape}")
