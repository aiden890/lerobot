...
try:  # Optional dependency used for language tokenization (SmolVLA, etc.)
    from transformers import AutoTokenizer  # type: ignore
except ImportError:  # pragma: no cover - transformers may be missing in minimal setups
...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def __init__(self, config: PolicyServerConfig):
        ...
        self._language_max_length = 48
        self._language_default_task = "Please follow the instructions."
    ...
    def _setup_language_support(self) -> None:
        """Initialise helpers for policies that expect language tokens (e.g. SmolVLA).

        Creates a tokenizer when available so that we can inject placeholder language
        tokens whenever the client does not provide any textual instruction.
        """
        ...
        # Determine desired token sequence length from the policy config.
        self._language_max_length = getattr(self.policy.config, "tokenizer_max_length", 48)

        tokenizer = None
        try:
            # Prefer the tokenizer packaged with the loaded model to avoid extra downloads.
            ...
            self.logger.warning("Failed to access SmolVLA processor tokenizer: %s", err)

        if tokenizer is None and AutoTokenizer is not None:
            tokenizer_name = getattr(self.policy.config, "vlm_model_name", None)
            if tokenizer_name:
                try:
                    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
                except Exception as err:  # pragma: no cover - network / cache errors
                    self.logger.warning("Could not load tokenizer '%s': %s", tokenizer_name, err)

        if tokenizer is None:
            self.logger.warning(
                "Language tokenizer not available; falling back to zeroed tokens for SmolVLA."
            )

        self._language_tokenizer = tokenizer

    def _ensure_language_features(self, observation: dict[str, Any]) -> dict[str, Any]:
        """Ensure that language-related observation keys exist for language-conditioned policies."""
        ...
        if not isinstance(task, str) or not task.strip():
            task = self._language_default_task
        ...
        if self._language_tokenizer is not None:
            encoded = self._language_tokenizer(
                task,
                max_length=self._language_max_length,
                truncation=True,
                padding="max_length",
                return_tensors="pt",
            )
            tokens = encoded["input_ids"].to(device)
            mask = encoded["attention_mask"].to(device=device, dtype=torch.bool)
        else:
            length = self._language_max_length or 48
            tokens = torch.zeros((1, length), dtype=torch.long, device=device)
            mask = torch.zeros((1, length), dtype=torch.bool, device=device)
            mask[..., 0] = True  # ensure at least one token is considered valid

        observation[OBS_LANGUAGE_TOKENS] = tokens
        observation[OBS_LANGUAGE_ATTENTION_MASK] = mask
        return observation
    ...
    def _prepare_observation(self, observation_t: TimedObservation) -> Observation:
        ...
        observation = self._ensure_language_features(observation)
